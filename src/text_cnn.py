...
class TextCNN:
    ...
    def __init__(self, sequence_length, num_classes, vocab_size,
        ...
        for i, kernel_height in enumerate(kernel_sizes):
            ...
            with tf.variable_scope(scope_name) as conv_sp:
                ...
                conv_output = self.init_conv(self.embedded_chars_expanded, self.sequence_length,
                                             ...
                                             scope_name="conv-{}".format(kernel_height),
                                             pooling='max_pooling', cnn_padding='VALID')
        ...
        # 5TH ADD A HIDDEN LAYER
        print('--------------------> ', self.h_drop)
        num_hidden_nodes = 20
        weights_hidden = tf.Variable(tf.random_normal([num_filters_total, num_hidden_nodes]))
        bias_hidden = tf.Variable(tf.random_normal([num_hidden_nodes]))
        preactivations_hidden = tf.add(tf.matmul(self.h_drop, weights_hidden), bias_hidden)
        self.h_drop = tf.nn.sigmoid(preactivations_hidden)
        print('--------------------> ', self.h_drop)
        print('-----------------------')
        ...
        with tf.name_scope("output"):
            W = tf.get_variable(
                "W",
                shape=[num_hidden_nodes, num_classes],
